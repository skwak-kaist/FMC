...
if __name__ == "__main__":
    ...
    parser.add_argument('--qpmap_dir', type=str, default=None, \
        help='qpmap file director')
    parser.add_argument('--input_file', type=str, default=None, \
        help='input file that contains a list of image file names')
    parser.add_argument('--vtm_ver', type=str, default='12.0', \
    ...
    elif  args.vtm_ver == 'dqp':
        commandVTM_base = '{}/EncoderApp_dqp -c ./configs/VTM/encoder_intra_vtm_dqp.cfg -c ./configs/VTM/option.cfg'.format(args.bin_dir)
    else:    
    ...
    if args.vtm_ver =='dqp' and args.dqp !=0 :
        dqp_vtm_flag = True
    else:
        dqp_vtm_flag = False
    ...
    qpmap_dir = os.path.abspath(args.qpmap_dir)
    ...
    for file_path in file_list.keys():
        ...
        for img_idx, file in enumerate(file_list[file_path]):
            ...
            if dqp_vtm_flag:
                fg_qp = paramVTM_QP
                bg_qp = fg_qp + args.dqp
                #paramVTM_QP_map = os.path.join(qpmap_dir, str(paramVTM_QP), file_name + '_000_qp_map.txt')
                paramVTM_QP_map = file_name
