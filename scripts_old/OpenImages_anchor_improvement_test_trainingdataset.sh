...
DATASET_DIR=$(realpath ./dataset/COCO)
ANNO_DIR=$DATASET_DIR/annotations_5k
OUTPUT_DIR=./output/COCO_Train
mkdir -p ${OUTPUT_DIR}
OUTPUT_DIR=$(realpath $OUTPUT_DIR)

## original source image path (jpg)
SOURCE_DATA_DIR=$DATASET_DIR/train2017
...
do
    ...
    jpg_input_list=$OUTPUT_DIR/file_list.lst

#    input_fname=$ANNO_DIR/${task}_validation_input_5k.lst
    input_fname=$DATASET_DIR/file_list.lst
    ...
    then
        ...
        do
            ...
            do
                scale=${DS2SCALE[ds]}
                task_info_list+=("QP${qp}_${scale}","${modified_data_dir}","${converted_data_dir}","${encoded_bitstream_dir}","${input_fname}",${qp},${ds})
            done
        done
