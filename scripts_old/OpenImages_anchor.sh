...
OUTPUT_DIR=./output/OpenImages_anchor
mkdir -p ${OUTPUT_DIR}
...
function process_conversion() {
    ...
    IFS=',' read param_task_id param_input_dir param_converted_dir param_bitstream_dir param_input_file param_qp param_ds <<< "${param_task_info}"
...
function process_evaluation() {
    ...
    local param_bounding_boxes=$ANNO_DIR/${param_task}_validation_bbox_5k.csv 
    local param_image_labels=$ANNO_DIR/${param_task}_validation_labels_5k.csv
    ...
    then
        ...
        local param_instance_segmentations=$ANNO_DIR/segmentation_validation_masks_5k.csv
        local param_segmentation_mask_dir=$ANNO_DIR/challenge_2019_validation_masks
...
do
    converted_data_dir=$OUTPUT_DIR/${task}/converted
    encoded_bitstream_dir=$OUTPUT_DIR/${task}/bitstream
    ...
    then
        ...
        do
            ...
            do
                scale=${DS2SCALE[ds]}
                task_info_list+=("QP${qp}_${scale}","${SOURCE_DATA_DIR}","${converted_data_dir}","${encoded_bitstream_dir}","${jpg_input_list}",${qp},${ds})
            done
        done
    ...
    then
        ...
        then
            ...
            task_info_list+=("uncmprd","${SOURCE_DATA_DIR}","${jpg_input_list}","${output_fname}",${cuda_dev})
            cuda_dev=$(($cuda_dev+1))
